 ...
 */
const saveFile = (path: string, data: any = "", { encoding = "utf-8", overwrite = true, newFile = true }: FileOptions = {}) => {
  path = setPath(path);
 ...
 */
const saveJson = (path: string, data = {}, { indent = 2, overwrite = true, newFile = false }: JsonOptions = {}) => {
 ...
 * @param pattern
 */
const findFiles = (folder: string, pattern: string | RegExp = "", arrayOfFiles: string[] = []) => {
